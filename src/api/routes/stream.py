...
async def signal_event_generator(
    ...
    try:
        while True:
            # Check latest signal (default demo pair BTC / swing)
            try:
                latest = await repo.get_latest_signal(
                    market_id=MarketId("binance:BTCUSDT"),
                    horizon=HorizonId.SWING_24H,
                )
...
async def stream_signals(
    ...
    return StreamingResponse(
        signal_event_generator(request=request, repo=repo, max_events=max_events),
